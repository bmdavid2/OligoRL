...
function runtimes(data)
    ...
    for i = 1:nrow(data)
        bases = [all_bases for _ in 1:data.oligo_lengths[i]]  
...
###################################
# Run this code to re-create the RL portion of the Figure 1 experiments. Requires running rebase.R code in the same directiory prior to this code.
# This code runs the experiments that are generated first in the rebase.R code. 
###################################



data = runtimes(CSV.read("./results_sites.csv",DataFrame))
CSV.write("./rollout_results_sites.csv",data)
algorithm=vcat(repeat(["CutFree"],nrow(data)),repeat(["RL"],nrow(data)))
time=vcat(data.time,data.random_runtime)
deg=vcat(data.degeneracy,data.random_objval)
nsites=vcat(data.n_sites,data.n_sites)
intdata=DataFrame(n_sites=nsites,algorithm=algorithm,time=time,degeneracy=deg)
CSV.write("./Cutfree_interaction_data.csv",intdata)
data = runtimes(CSV.read("./results_length.csv",DataFrame))
CSV.write("./rollout_results_length.csv",data)
